...
/**
 * Error khusus untuk validasi form (agar UI bisa bedakan vs error API).
 * Message default tetap "VALIDATION_ERROR" supaya backward-compatible.
 */
...
> {
  ...
  touchMany(keys: (keyof TValues)[]) {
    if (!keys.length) return;
    const next: Partial<Record<keyof TValues, boolean>> = { ...this.touched };
  ...
  async submit(
    ...
    if (hasAnyError<TValues>(merged as ErrorMap<TValues>)) {
      const bad: (keyof TValues)[] = [];
      for (const k of Object.keys(merged) as Array<keyof TValues>) {
        if (merged[k]) bad.push(k);
      }
      // pastikan error tampil (banyak komponen Field.Error hanya render jika touched)
      this.touchMany(bad);
...
//     console.log("merged Validate : ",merged);
//     console.log("haseAnyError : ",hasAnyError<TValues>(merged as ErrorMap<TValues>))
