...
export default function FleetFormPage({
  ...
  function openErrorDialog(err: unknown) {
    ...
    const msg =
      String(rawMsg).trim() === "VALIDATION_ERROR"
        ? (t("common.validation_error") ?? "Mohon periksa kembali field yang wajib diisi.")
        : rawMsg;

    setDlgKind("error");
    setDlgTitle(t("common.failed_to_save") ?? "Gagal menyimpan");
    setDlgMsg(
      <pre className="whitespace-pre-wrap text-xs text-red-700">{String(msg)}</pre>
    );
    setDlgOpen(true);
  }
  ...
  async function onSave() {
    // Validasi dulu sebelum upload (biar tidak buang request).
    const errs = ctrl.validate();
    const bad = (Object.keys(errs) as Array<keyof FleetValues>).filter((k) => !!errs[k]);
    if (bad.length) {
      ctrl.touchMany(bad);
      openErrorDialog(new Error("VALIDATION_ERROR"));
      return;
    }
    let saved = false;
    let unitIdToSync = 0;
    ...
    try {
      ...
      if (mode === "create") {
        ...
        console.log("NEW UNIT ID TO SYNC : ", unitIdToSync);
