...
import { odooUtcToDatetimeLocalValue, userLocalToOdooUtc } from "@/lib/datetime";
...
const DOC_ATTACHMENT_PICKUP_TYPE =
  "transport_order_pickup_attachment" as const;
const DOC_ATTACHMENT_DROP_OFF_TYPE =
  "transport_order_drop_off_attachment" as const;
...
export default function OrdersCreateForm<T extends TmsUserType>({
  ...
  async function uploadDocumentsForDocTypeGroup(
    ...
    if (!res.ok) {
      const t = await res.text().catch(() => "");
      throw new Error(
        `Gagal upload dokumen (HTTP ${res.status}). ${t}`.trim()
      );
    }
    ...
    if (maybe && typeof maybe.id === "number" && Array.isArray(maybe.attachments)) {
...
//         openChatErrorDialog(
//           msg || (t("common.failed_save") ?? "Gagal menyimpan.")
//         );
//         return;
//       }
//       setChatMsg("");
//       openChatSuccessDialog(
//         t("orders.message_sent") ?? "Pesan terkirim.",
...
//         t("common.network_error") ?? "Network Error"
//       );
//     } finally {
...
// const DOC_ATTACHMENT_DROPOFF_TYPE = "route_transport_drop_off" as const;
...
//   async function uploadDocumentsForDocTypeGroup(
//     files: File[],
//     opts: { docType: string; groupId?: number | null }
//   ): Promise<OrderAttachmentGroup> {
//     const baseUrl = DOCUMENT_ATTACHMENTS_URL.replace(/\/$/, "");
//     const url = opts.groupId
//       ? `${baseUrl}/${opts.groupId}`
//       : `${baseUrl}?doc_type=${encodeURIComponent(opts.docType)}`;
...
//     const ct = res.headers.get("content-type") || "";
//     if (ct.includes("application/json"))
//       return (await res.json()) as OrderAttachmentGroup;

//     // fallback GET group
//     if (opts.groupId) {
//       const gRes = await fetch(`${baseUrl}/${opts.groupId}`, {
//         credentials: "include",
//       });
//       if (gRes.ok) return (await gRes.json()) as OrderAttachmentGroup;
//     }
...
//   const uploadPickupAttachmentGroup = async (
//     files: File[],
//     groupId?: number | null
//   ) => {
//     const gid =
...
//       pickupAttachment?.id ??
//       null;
//     const group = await uploadDocumentsForDocTypeGroup(files, {
//       docType: DOC_ATTACHMENT_PICKUP_TYPE,
//       groupId: gid,
//     });
//     setPickupAttachment(group);
//     return group;
//   };

//   const removePickupAttachmentItem = async (
//     groupId: number,
//     attachmentId: number
//   ) => {
//     await deleteRemoteAttachment(groupId, attachmentId);
//     setPickupAttachment((prev) =>
//       prev
//         ? ({
//             ...(prev as any),
//             attachments: (prev as any).attachments?.filter(
...
//   const removeDropOffAttachmentItem = async (
//     groupId: number,
//     attachmentId: number
//   ) => {
//     await deleteRemoteAttachment(groupId, attachmentId);
//     setDropOffAttachment((prev) =>
//       prev
//         ? ({
//             ...(prev as any),
//             attachments: (prev as any).attachments?.filter(
...
//   const uploadDropOffAttachmentGroup = async (
//     files: File[],
//     groupId?: number | null
//   ) => {
//     const gid =
...
//       null;
//     const group = await uploadDocumentsForDocTypeGroup(files, {
//       docType: DOC_ATTACHMENT_DROPOFF_TYPE,
//       groupId: gid,
//     });
//     setDropOffAttachment(group);
//     return group;
//   };
...
//                 pickupAttachment={pickupAttachment}
//                 setPickupAttachment={setPickupAttachment}
//                 uploadPickupAttachment={uploadPickupAttachmentGroup}
//                 removePickupAttachmentFile={removePickupAttachmentItem}
//                 dropOffAttachment={dropOffAttachment}
//                 setDropOffAttachment={setDropOffAttachment}
//                 uploadDropOffAttachment={uploadDropOffAttachmentGroup}
//                 removeDropOffAttachmentFile={removeDropOffAttachmentItem}
//               />
